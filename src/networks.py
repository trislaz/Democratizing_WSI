...
from torch.nn import (AvgPool2d, BatchNorm1d, BatchNorm2d, Conv1d, Conv2d,
                      ...
                      Module, ModuleList, MultiheadAttention, ReLU, Sequential,
                      Sigmoid, Softmax, Tanh, functional)
...
class ResidualMLPBlock(Module):
    ...
    def __init__(self, in_features, out_features=None, dropout=0, bn=True):
        ...
        if out_features is None:
            ...
            self.residual_lin = LinearBatchNorm(in_features, out_features, dropout, constant_size=self.bn, relu=False)
...
class ResidualMLP(Module):
    def __init__(self, args):
        ...
        tile_encoder = []
        tile_encoder.append(ResidualMLPBlock(self.feature_depth, 512, dropout=self.dropout, bn=self.bn))
...
class LinearBatchNorm(Module):
    ...
    """
    def __init__(self, in_features, out_features, dropout, constant_size=True, dim_batch=None, relu=True):
...
def get_resnet_model(resnet_architecture: str, pretrained: Union[bool, str]):
    ...
    resnet_model = getattr(resnet_factory, resnet_architecture.lower())(False)#imagenet_pretrained, progress=True)
...
class FullSparseConvMIL(nn.Module):
    def __init__(self, args):
        super(FullSparseConvMIL, self).__init__()
        self.args = args
        resnet_model, _ = get_resnet_model('resnet18', 'imagenet')
        _, sparse_subresnet = cut_resnet_dense_sparse(resnet_model, 4)
        pooling_model = SparseConvMIL(sparse_subresnet, args.downmap, False) #downsample 500 before
        n_classes = args.num_class
        linear_classifier = torch.nn.Linear(512, n_classes, False)
        freeze_pooling_model = 1
        whole_model = LinearWithMIL(pooling_model, linear_classifier, freeze_pooling_model)
